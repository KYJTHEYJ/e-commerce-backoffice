...
public class GlobalExceptionHandler<T> {
    ...
    public ResponseEntity<MessageResponse> handleServiceErrorException(ServiceErrorException e) {
        log.error(e.getMessage(), e);
        return ResponseEntity.status(e.getHttpStatus()).body(MessageResponse.fail(e.getHttpStatus().name(), e.getMessage()));
    }

    @ExceptionHandler(value = IllegalArgumentException.class)
    public ResponseEntity<MessageResponse> IllegalArgumentExceptionHandler(IllegalArgumentException e) {
    ...
    public ResponseEntity<MessageResponse> HttpMessageNotReadableException(HttpMessageNotReadableException e) {
        log.error("데이터 JSON 변환 에러 발생 : ", e);
        return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(MessageResponse.fail(HttpStatus.BAD_REQUEST.name(), MSG_NOT_VALID_VALUE));
    }

    @ExceptionHandler(value = DataIntegrityViolationException.class)
    public ResponseEntity<MessageResponse> DataIntegrityViolationExceptionHandler(DataIntegrityViolationException e) {
    ...
    public ResponseEntity<MessageResponse> handleCriticalErrorException(Exception e) {
